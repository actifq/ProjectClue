...
public class ClueMain extends JFrame implements ActionListener, KeyListener, Runnable, MouseListener, FocusListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		// ################## GameWaitngRoom // gwr 160217 찬재추가
			...
			/*
			 * repaint(); card.previous(getContentPane());
			 * card.show(getContentPane(), "MS");
			 * 
			 * mainScreen.game.savePlayerStatus();
			 * mainScreen.game.setGamePlayer(Game.crrPlayer,
			 * mainScreen.game.runDice());
			 * 
			 * mainScreen.showCount(); mainScreen.setImage();
			...
			try {
				...
				out.write((Function.HINT + "|" + myRoom + "|" + mainScreen.game.p[pGiveHint % 4].id + "|"
						+ (pGiveHint + 1) % 4 + "|" + sr + "|" + sp + "|" + sw + "|" + hint + "\n").getBytes());
			} catch (Exception ex) {
			...
			/*
			 * need to decide the action after guessing. either show up one
			 * dialog or either give a msg on chatarea
			 */
		...
		// ############## cs/ 무기
			...
			try {
				out.write((Function.CHOOSECARD + "|" + 6 + "\n").getBytes());
			} catch (Exception ex) {
			}

		} else if (e.getSource() == cs.q[1]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 7 + "\n").getBytes());
			} catch (Exception ex) {
			}
		} else if (e.getSource() == cs.q[2]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 8 + "\n").getBytes());
			} catch (Exception ex) {
			}
		} else if (e.getSource() == cs.q[3]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 9 + "\n").getBytes());
			} catch (Exception ex) {
			}
		} else if (e.getSource() == cs.q[4]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 10 + "\n").getBytes());
			} catch (Exception ex) {
			}
		} else if (e.getSource() == cs.q[5]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 11 + "\n").getBytes());
			} catch (Exception ex) {
			}
		} else if (e.getSource() == cs.q[6]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 12 + "\n").getBytes());
			} catch (Exception ex) {
			}
		} else if (e.getSource() == cs.q[7]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 13 + "\n").getBytes());
			} catch (Exception ex) {
			}
		}
		...
		// ########### 방
			...
			try {
				out.write((Function.CHOOSECARD + "|" + 14 + "\n").getBytes());
			} catch (Exception ex) {
			}
		} else if (e.getSource() == cs.j[1]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 15 + "\n").getBytes());
			} catch (Exception ex) {
			}
		} else if (e.getSource() == cs.j[2]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 16 + "\n").getBytes());
			} catch (Exception ex) {
			}
		} else if (e.getSource() == cs.j[3]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 17 + "\n").getBytes());
			} catch (Exception ex) {
			}
		} else if (e.getSource() == cs.j[4]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 18 + "\n").getBytes());
			} catch (Exception ex) {
			}
		} else if (e.getSource() == cs.j[5]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 19 + "\n").getBytes());
			} catch (Exception ex) {
			}
		} else if (e.getSource() == cs.j[6]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 20 + "\n").getBytes());
			} catch (Exception ex) {
			}
		} else if (e.getSource() == cs.j[7]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 21 + "\n").getBytes());
			} catch (Exception ex) {
			}
		} else if (e.getSource() == cs.j[8]) {
			try {
				out.write((Function.CHOOSECARD + "|" + 22 + "\n").getBytes());
			} catch (Exception ex) {
			}
		}
	...
	public void keyTyped(KeyEvent e) {
		// TODO Auto-generated method stub
		/*
		 * if(myNum==Game.crrPlayer){//내가 현재 플레이어일때. int key=-1;
		 * 
		 * //mainScreen.game.gp.keyPressed(e);
		 * 
		 * switch(e.getKeyCode()){ case KeyEvent.VK_RIGHT: key=3; break; case
		 * KeyEvent.VK_LEFT: key=2; break; case KeyEvent.VK_UP: key=0; break;
		 * case KeyEvent.VK_DOWN: key=1; break; }
		 * 
		 * try{
		 * 
		...
		while (true) {
			try {
				...
				case Function.ROOMOUT: {
					...
					for (int i = 0; i < 4; i++) {
						...
						if (mid.equals(id)) {
							...
							gwr.aa[i].add("Center", new JLabel(new ImageIcon("image/back/qcard.png")));
							gwr.aa[i].validate();
						}
					}
				}
				...
				case Function.AVATA:
					...
					gwr.avaName[pNum - 1].setText(RefData.nameChar[charNum - 1]);// 캐릭터
																					// 바꾸기
				...
				case Function.SELECTCARD: {
					...
					cs.guess[0].add(new JLabel(new ImageIcon(setImage("image/room/room" + (roomNo - 1) + ".jpg",
							cs.guess[0].getWidth(), cs.guess[0].getHeight()))));
					cs.guess[0].validate();// panel재배치

					cs.pl.removeAll();
					cs.pl.add(new JLabel(new ImageIcon(setImage("image/player/char" + (avata - 1) + ".jpg",
							cs.pl.getWidth(), cs.pl.getHeight()))));
					...
					if (myNum == Game.crrPlayer) {
						...
						if (n != 0) {
							...
							try {
								...
								out.write((Function.REACHROOM + "|" + myRoom + "|" + (myNum + 1) + "|"
										+ RefData.nameRoom[n - 1] + "\n").getBytes());
							} catch (Exception ex) {
				...
				case Function.CHOOSECARD: {
					...
					try {
						if (cardnum < 6) {
							...
							cs.guess[1].add(new JLabel(new ImageIcon(setImage("image/player/char" + cardnum + ".jpg",
									cs.guess[0].getWidth(), cs.guess[0].getHeight()))));
							...
							cardnum = cardnum - 13;
							cs.guess[0].removeAll();
							cs.guess[0].add(new JLabel(new ImageIcon(setImage("image/room/room+" + cardnum + ".jpg",
									cs.guess[2].getWidth(), cs.guess[2].getHeight()))));
							cs.guess[0].validate();// panel재배치
							cs.tfGuess[0].setText(RefData.nameRoom[cardnum]);
						} else {
							cardnum = cardnum - 6;
							cs.guess[2].removeAll();
							cs.guess[2].add(new JLabel(new ImageIcon(setImage("image/weapon/wp" + cardnum + ".jpg",
									cs.guess[0].getWidth(), cs.guess[0].getHeight()))));
	...
	public void focusGained(FocusEvent e) {
		/*
		 * if(e.getSource()==mainScreen.jpGameBoard){ setFocusable(true); }
		 */
	}
	...
	public void focusLost(FocusEvent e) {
		// TODO Auto-generated method stub
		/*
		 * if(e.getSource()==mainScreen.jpGameBoard){ setFocusable(false); }
		 */
	}

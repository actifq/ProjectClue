...
public class Server implements Runnable {
	...
	class ClientThread extends Thread {
			...
			while (true) {
				try {
					...
					case Function.MAKEROOM: {
						...
						messageAll(Function.MAKEROOM + "|" + room.roomName + "|" + room.roomState + "|" + room.current
								+ "/" + room.maxcount);
					...
					case Function.ROOMIN: {
						 ...
						 * 사람들에게 개인정보 전송 입장메세지 전송 5) userVc 에 추가 6) 방에 입장하라는 메세지
						 * 전송 7) 방에 들어가 있는 모든 사람의 정보를 받는다 8) 대기실 갱신을 한다
						 */
						...
						for (int i = 0; i < roomVc.size(); i++) {
							...
							if (rname.equals(room.roomName))// 방을 찾는다
							{
								...
								for (int j = 0; j < room.userVc.size(); j++) {
									...
									// c.messageTo(Function.ROOMCHAT+"|[알림]
									// "+id+"님이 입장하셨습니다");
								}
								// 본인(방에 들어가는 사람)
								/*
								 * messageTo(Function.ROOMIN+"|"+id+"|"+sex+"|"+
								 * room.roomName); room.userVc.addElement(this);
								 */
								for (int j = 0; j < room.userVc.size(); j++) {
								...
								// 대기실 처리
								messageAll(Function.WAITROOMUPDATE + "|" + id + "|" + pos + "|" + room.roomName + "|"
										+ room.current + "|" + room.maxcount);
								// 6/6
							}
						}
					...
					case Function.ROOMOUT: {
						...
						/*
						 * * 1) 방이름을 받는다 2) 방을 찾는다 3) ==> 현재인원감소 ==> 위치변경(대기실)
						 * 4) 방에 있는 사람들에게 삭제명령 퇴장메세지 전송 5) userVc에 삭제 6) 대기실로 변환
						 * 메세지 전송
						 * 
						 * 7) 대기실 갱신을 한다
						 */
					...
					case Function.READY: {
						...
						for (int i = 0; i < roomVc.size(); i++) {
							...
							for (int j = 0; j < room.userVc.size(); j++) {
								...
								// c.messageTo(Function.ROOMCHAT+"|[알림] "+id+"님이
								// 준비하였습니다");
...

					{
						...
						for (int i = 0; i < roomVc.size(); i++) {
							...
							for (int j = 0; j < room.userVc.size(); j++) {
								ClientThread c = room.userVc.elementAt(j);
								c.messageTo(Function.STARTGAME + "|" + c.pnum + "|" + c.avata + "|" + room.getAnsCard()
										+ room.getPCard());// 자신의 Player 넘버와 캐릭터
					...
					case Function.HINT: {
						...
						for (int i = 0; i < roomVc.size(); i++) {
							...
							for (int j = 0; j < room.userVc.size(); j++) {
								...
								if (c.id.equals(id)) {
									messageTo(Function.MYHINT + "|" + pAva + "|" + pp + "|" + r + "|" + p + "|" + w
											+ "|" + hint);
									continue;
								}

								c.messageTo(Function.HINT + "|" + pAva + "|" + pp + "|" + r + "|" + p + "|" + w);
								c.messageTo(Function.ROOMCHAT + "|[알림] " + pAva + "(" + pp + "P)님이" + r + "," + p + ","
										+ w + "중 하나의 카드를 보여주었습니다.");
		...
		public synchronized void messageTo(String msg) {// 순차적으로 보내야하니까 동기화로.
			try {
				out.write((msg + "\n").getBytes());// 데이터 넘길 때 쓰는 것,.바이트로 넘김.
													// 역슬래쉬로 끝냄
